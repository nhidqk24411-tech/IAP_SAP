...
try:
    ...
    print(f"⚠️ Không thể import config: {e}")
    # Tạo config mặc định
    class Config:
...
class PerformanceDashboard(QWidget):
    """Dashboard displaying employee performance - Supports year/month filtering"""

    def __init__(self, user_name, year=None, month=None, controller=None):
        super().__init__()
    ...
    def init_ui(self):
        ...
        if self.filter_year and self.filter_month:
            title = f"📊 PERFORMANCE DASHBOARD - {self.user_name} (Month {self.filter_month}/{self.filter_year})"
        elif self.filter_year:
            title = f"📊 PERFORMANCE DASHBOARD - {self.user_name} (Year {self.filter_year})"
        else:
            title = f"📊 PERFORMANCE DASHBOARD - {self.user_name} (All Years)"
        ...
        title_label = QLabel(f"Employee Performance Overview - {self.user_name}")
        title_label.setStyleSheet("""
                    font-size: 24px;
                    font-weight: 700;
                    color: #ffffff;
    ...
    def create_metrics_grid(self):
        """Tạo grid hiển thị 4 chỉ số chính - Cập nhật tiêu đề dựa trên bộ lọc"""
        if self.filter_year and self.filter_month:
            title = f"CHỈ SỐ HIỆU SUẤT (THÁNG {self.filter_month}/{self.filter_year})"
        elif self.filter_year:
            title = f"CHỈ SỐ HIỆU SUẤT (NĂM {self.filter_year})"
        else:
            title = "CHỈ SỐ HIỆU SUẤT (CẢ NĂM)"

        grid = QGroupBox(title)
    ...
    def load_data(self):
        """Load data from DataProcessor according to filter"""
        try:
            print(f"\n{'=' * 70}")

            if self.filter_year and self.filter_month:
                print(f"📊 LOADING DATA FOR {self.user_name} (MONTH {self.filter_month}/{self.filter_year})")
            elif self.filter_year:
                print(f"📊 LOADING DATA FOR {self.user_name} (YEAR {self.filter_year})")
            else:
                print(f"📊 LOADING DATA FOR {self.user_name} (LAST 4 YEARS)")
            ...
            # Load data WITH FILTER for charts
            chart_success = False

            # Use load_period_data with both year and month
            chart_success = self.data_processor.load_period_data(self.filter_year, self.filter_month)

            if chart_success:
                self.year_data = self.data_processor.get_dashboard_data()

            if not chart_success:
                print("❌ Không thể tải dữ liệu biểu đồ từ DataProcessor")
                QMessageBox.warning(self, "Lỗi", "Không thể tải dữ liệu từ hệ thống")
                return

            # TẢI DỮ LIỆU CẢ NĂM CHO REPORT (LUÔN LÀ NĂM HIỆN TẠI)
            current_year = datetime.now().year
            report_success = False

            if current_year == self.filter_year and not self.filter_month:
                # Nếu đang xem cả năm hiện tại, dùng chung dữ liệu
                self.full_year_data = self.year_data
                report_success = True
            else:
                # Tải dữ liệu cả năm hiện tại riêng cho report
                report_processor = DataProcessor(self.user_name)
                report_success = report_processor.load_year_data(current_year)
                if report_success:
                    self.full_year_data = report_processor.get_dashboard_data()

            if not report_success:
                print("⚠️ Không thể tải dữ liệu cả năm cho report")
                # Vẫn tiếp tục với dữ liệu hiện có

            # Tính toán metrics cho biểu đồ (theo bộ lọc)
            self.calculate_dashboard_metrics()

            # Tính toán metrics cho report (luôn cả năm hiện tại)
    ...
    def calculate_report_metrics(self):
        """Tính toán metrics cho report (luôn lấy dữ liệu cả năm hiện tại)"""
        try:
            ...
            report_title = f"PHÂN TÍCH HIỆU SUẤT NĂM {current_year} (TỪ {month_names[0]} ĐẾN {month_names[current_month - 1]})"
            ...
            if not self.full_year_data:
                print("⚠️ Không có dữ liệu cả năm cho report")
                # Sử dụng dữ liệu hiện có nếu không có full_year_data
                self.full_year_data = self.year_data
                if not self.full_year_data:
                    print("❌ Không có dữ liệu nào cho report")
                    # Vẫn tạo report_metrics với giá trị mặc định
            else:
                # Lấy dữ liệu từ full_year_data
                sap_sheets = self.full_year_data.get('sap_data', {}).get('sheets', {})
                work_log_sheets = self.full_year_data.get('work_log', {}).get('sheets', {})

                # 1. Tổng hợp dữ liệu orders
                orders_df = pd.DataFrame()
                if 'Orders' in sap_sheets and sap_sheets['Orders'] is not None:
                    orders_df = sap_sheets['Orders']
                    # Lọc chỉ lấy các tháng đã qua
                    if 'Month' in orders_df.columns:
                        orders_df = orders_df[orders_df['Month'] <= current_month]

                # 2. Tổng hợp dữ liệu fraud events
                ...
                if 'Fraud_Events' in work_log_sheets and work_log_sheets['Fraud_Events'] is not None:
                    ...
                    if 'Month' in fraud_df.columns:
                        fraud_df = fraud_df[fraud_df['Month'] <= current_month]

                # 3. Tổng hợp dữ liệu browser sessions
                ...
                if 'Browser_Sessions' in work_log_sheets and work_log_sheets['Browser_Sessions'] is not None:
                    ...
                    if 'Month' in browser_df.columns:
                        browser_df = browser_df[browser_df['Month'] <= current_month]
                ...
                # 4. Tổng hợp dữ liệu daily performance
                ...
                if 'Daily_Performance' in sap_sheets and sap_sheets['Daily_Performance'] is not None:
                    ...
                    if 'Month' in daily_df.columns:
                        daily_df = daily_df[daily_df['Month'] <= current_month]
                ...
                # Tính toán các chỉ số khác cho report
                if total_revenue > 0:
                    profit_margin = (total_profit / total_revenue * 100)

                # Hiệu suất thời gian (giả định mục tiêu 8h/ngày, 22 ngày/tháng)
                target_hours_per_month = 176  # 8h * 22 ngày
                if months_passed > 0:
                    time_efficiency = (total_work_hours / (target_hours_per_month * months_passed) * 100)

                # Tỷ lệ lỗi (dựa trên fraud events)
                if total_orders > 0:
                    error_rate = (total_fraud / total_orders * 100)

                # Đơn hàng/giờ
                if total_work_hours > 0:
                    orders_per_hour = total_orders / total_work_hours
            ...
            avg_monthly_orders = total_orders / months_passed if months_passed > 0 else 0
            avg_monthly_hours = total_work_hours / months_passed if months_passed > 0 else 0
            avg_monthly_revenue = total_revenue / months_passed if months_passed > 0 else 0
            avg_monthly_profit = total_profit / months_passed if months_passed > 0 else 0
            ...
            print(f"📊 Đã tính toán report metrics cho năm {current_year} ({months_passed} tháng)")

        except Exception as e:
            print(f"❌ Lỗi tính toán report metrics: {e}")
            import traceback
            traceback.print_exc()

            # Vẫn tạo report_metrics với giá trị mặc định để tránh lỗi
            current_date = datetime.now()
            current_year = current_date.year
            current_month = current_date.month

            self.report_metrics = {
                'report_title': f"PHÂN TÍCH HIỆU SUẤT NĂM {current_year}",
                'current_year': current_year,
                'current_month': current_month,
                'months_passed': current_month,
                'month_names': ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12'],
    ...
    def calculate_dashboard_metrics(self):
        """Tính toán các chỉ số cho dashboard từ dữ liệu"""
        try:
            ...
            if self.filter_year and self.filter_month:
                ...
                target_month = int(self.filter_month)
                months_range = [target_month]
            else:
    ...
    def update_chart_titles(self):
        """Cập nhật tiêu đề biểu đồ dựa trên bộ lọc"""
        try:
            # Lấy reference đến các group box
            for child in self.findChildren(QGroupBox):
                title = child.title()
                if "THEO THÁNG" in title:
                    if self.filter_year and self.filter_month:
                        new_title = title.replace("THEO THÁNG", f"THÁNG {self.filter_month}/{self.filter_year}")
                    elif self.filter_year:
                        new_title = title.replace("THEO THÁNG", f"NĂM {self.filter_year}")
                    else:
                        new_title = title.replace("THEO THÁNG", "CẢ NĂM")
                    child.setTitle(new_title)
        except Exception as e:
            print(f"⚠️ Lỗi cập nhật tiêu đề biểu đồ: {e}")
    ...
    def _calculate_completion_distribution(self, orders_df):
        """Tính toán phân bổ mức độ hoàn thành từ dữ liệu"""
        try:
            ...
            def normalize_status(status):
                ...
                if any(word in status_str for word in ['hoàn thành', 'completed', 'done', 'finished']):
                    return 'Hoàn thành'
                elif any(word in status_str for word in ['đang xử lý', 'processing', 'in progress', 'đang làm']):
                    return 'Đang xử lý'
                else:
                    return 'Chưa bắt đầu'

            # Áp dụng chuẩn hóa
            normalized_status = orders_df['Status'].apply(normalize_status)

            # Đếm số lượng theo trạng thái
            status_counts = normalized_status.value_counts()

            # Đảm bảo có đủ 3 loại
            completed = status_counts.get('Hoàn thành', 0)
            processing = status_counts.get('Đang xử lý', 0)
            pending = status_counts.get('Chưa bắt đầu', 0)

            # Nếu có status khác, phân loại vào pending
            other_status = len(orders_df) - (completed + processing + pending)
            pending += other_status
    ...
    def update_ui(self):
        ...
        try:
            ...
            if self.filter_year and self.filter_month:
                self.setWindowTitle(f"📊 Dashboard Hiệu Suất - {self.user_name} (Tháng {self.filter_month}/{self.filter_year}) | {current_time}")
            elif self.filter_year:
                self.setWindowTitle(f"📊 Dashboard Hiệu Suất - {self.user_name} (Năm {self.filter_year}) | {current_time}")
            else:
    ...
    def update_completion_chart(self):
        ...
        try:
            ...
            total_orders = completion_data.get('total', 0)

            # Tạo tiêu đề dựa trên bộ lọc
            if self.filter_year and self.filter_month:
                chart_title = f'Phân Bổ Mức Độ Hoàn Thành - Tháng {self.filter_month}/{self.filter_year}'
            elif self.filter_year:
                chart_title = f'Phân Bổ Mức Độ Hoàn Thành - Năm {self.filter_year}'
            else:
                current_month = datetime.now().month
                chart_title = f'Phân Bổ Mức Độ Hoàn Thành - Từ T1 đến T{current_month}'

            # Chỉ vẽ biểu đồ nếu có dữ liệu
            if sum(sizes) > 0 and total_orders > 0:
                # Tạo biểu đồ tròn với khoảng cách giữa các phần
                wedges, texts, autotexts = ax.pie(
                    sizes,
                    labels=labels,
                    colors=colors,
                    autopct=lambda pct: f'{pct:.1f}%\n({int(pct / 100 * total_orders):.0f} đơn)',
                    startangle=90,
                    pctdistance=0.85,
                ...
                # Thêm thông tin tổng số đơn hàng ở giữa
                if total_orders > 0:
                    ax.text(0, 0, f'Tổng: {total_orders:,}\nđơn hàng',
                            ha='center', va='center',
                ...
                if event.inaxes == ax and sum(sizes) > 0:
                    for i, wedge in enumerate(wedges):
                        if wedge.contains_point((event.x, event.y)):
                            ...
                            # Tạo annotation mới
                            self.completion_annotation = ax.annotate(
                                f'{labels[i]}\n{order_count:.0f} đơn ({percentage:.1f}%)',
                                xy=(0, 0),
                                xytext=(20, 20),
                                textcoords='offset points',
                                ha='left',
    ...
    def update_analysis_text(self):
        ...
        try:
            # Sử dụng report_metrics thay vì metrics
            if not self.report_metrics:
                print("⚠️ Không có report metrics, sử dụng metrics thông thường")
                metrics = self.metrics
                # Tạo các biến giả lập cho report
                current_year = datetime.now().year
                current_month = datetime.now().month
                month_names = ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12']
                months_passed = current_month
                ...
                current_year = metrics.get('current_year', datetime.now().year)
                current_month = metrics.get('current_month', datetime.now().month)
                month_names = metrics.get('month_names',
                                          ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12'])
                months_passed = metrics.get('months_passed', current_month)
                report_title = metrics.get('report_title', f"PHÂN TÍCH HIỆU SUẤT NĂM {current_year}")

            # Lấy các giá trị từ metrics
            total_orders = metrics.get('total_orders', 0)
            completed_orders = metrics.get('completed_orders', 0)
            completion_rate = metrics.get('completion_rate', 0)
            total_work_hours = metrics.get('total_work_hours', 0)
            ...
            avg_monthly_orders = metrics.get('avg_monthly_orders', 0)
            avg_monthly_hours = metrics.get('avg_monthly_hours', 0)
            avg_monthly_revenue = metrics.get('avg_monthly_revenue', 0)
            avg_monthly_profit = metrics.get('avg_monthly_profit', 0)
            pending_orders = metrics.get('pending_orders', 0)

            # Tính toán các giá trị bổ sung NẾU CHƯA CÓ
            revenue_per_order = metrics.get('revenue_per_order', 0)
            if revenue_per_order == 0 and total_orders > 0:
                revenue_per_order = total_revenue / total_orders

            profit_per_order = metrics.get('profit_per_order', 0)
            if profit_per_order == 0 and total_orders > 0:
                profit_per_order = total_profit / total_orders
            ...
            risk_level = ""
            risk_analysis = ""
            avg_fraud_per_month = total_fraud / months_passed if months_passed > 0 else 0

            if total_fraud > 10 or critical_count > 5 or avg_fraud_per_month > 2:
                risk_level = "Cao"
                risk_analysis = f"Có {total_fraud} sự kiện gian lận ({avg_fraud_per_month:.1f}/tháng) và {critical_count} cảnh báo nghiêm trọng. Cần xem xét và xử lý ngay."
            elif total_fraud > 5 or critical_count > 3 or avg_fraud_per_month > 1:
                risk_level = "Trung bình"
                risk_analysis = f"Có {total_fraud} sự kiện gian lận ({avg_fraud_per_month:.1f}/tháng) và {critical_count} cảnh báo nghiêm trọng. Cần giám sát chặt chẽ."
            else:
                risk_level = "Thấp"
                risk_analysis = f"Rủi ro ở mức chấp nhận được với {total_fraud} sự kiện gian lận ({avg_fraud_per_month:.1f}/tháng)."

            # Tạo HTML report
            analysis_html = f"""
            <div style="color: #cbd5e1; font-family: 'Segoe UI', Arial, sans-serif;">
                <h3 style="color: #ffffff; margin-bottom: 15px;">{report_title}</h3>

                <div style="background-color: #1e293b; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #3b82f6;">
                    <h4 style="color: #3b82f6; margin-top: 0; margin-bottom: 10px;">📅 THỜI GIAN PHÂN TÍCH</h4>
                    <p style="margin: 5px 0; color: #94a3b8;">
                        <span>Phân tích từ: </span>
                        <span style="color: #ffffff; font-weight: 600;">{month_names[0]}/{current_year}</span>
                        <span> đến: </span>
                        <span style="color: #ffffff; font-weight: 600;">{month_names[current_month - 1]}/{current_year}</span>
                    </p>
                    <p style="margin: 5px 0; color: #94a3b8;">
                        <span>Tổng số tháng: </span>
                        <span style="color: #ffffff; font-weight: 600;">{months_passed} tháng</span>
                        <span> | Ngày tạo báo cáo: </span>
                        <span style="color: #ffffff; font-weight: 600;">{datetime.now().strftime('%d/%m/%Y %H:%M')}</span>
                    </p>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    ...
                    <div style="background-color: #1e293b; padding: 15px; border-radius: 8px; border-left: 4px solid #10b981;">
                        ...
                        <p style="margin: 5px 0;">
                            <span style="color: #94a3b8;">Doanh thu:</span> 
                            <span style="color: #ffffff; font-weight: 600;">{total_revenue:,.0f} VND</span>
                        </p>
                        <p style="margin: 5px 0;">
                            <span style="color: #94a3b8;">Lợi nhuận:</span> 
                        ...
                        <p style="margin: 5px 0;">
                            <span style="color: #94a3b8;">Lợi nhuận/đơn hàng:</span> 
                            <span style="color: #10b981; font-weight: 600;">{profit_per_order:,.0f} VND</span>
                        </p>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    ...
                    <div style="background-color: #1e293b; padding: 15px; border-radius: 8px; border-left: 4px solid #ef4444;">
                        ...
                        <p style="margin: 5px 0;">
                            ...
                            <span style="color: #94a3b8; font-size: 12px;"> ({avg_fraud_per_month:.1f}/tháng)</span>
                        </p>
                        <p style="margin: 5px 0;">
                        ...
                        <p style="color: #{'#ef4444' if risk_level == 'Cao' else '#f59e0b' if risk_level == 'Trung bình' else '#10b981'}; font-size: 12px; margin-top: 10px; padding: 10px; background-color: rgba(255,255,255,0.05); border-radius: 5px;">
                            {risk_analysis}
                        </p>
                    </div>
                    ...
                    <div style="background-color: #1e293b; padding: 15px; border-radius: 8px; border-left: 4px solid #8b5cf6;">
                        ...
                        <p style="margin: 5px 0;">
                            ...
                            {'<span style="color: #10b981;"> ✓ Đạt mục tiêu</span>' if completion_rate >= 95 else '<span style="color: #f59e0b;"> ⚠ Cần cải thiện</span>' if completion_rate >= 70 else '<span style="color: #ef4444;"> ✗ Dưới mục tiêu</span>'}
                        </p>
                        <p style="margin: 5px 0;">
                        ...
                        <p style="margin: 5px 0;">
                            <span style="color: #94a3b8;">Đánh giá tổng thể:</span> 
                            <span style="color: #{'#10b981' if performance_level in ['Xuất sắc', 'Tốt'] else '#f59e0b' if performance_level == 'Trung bình' else '#ef4444'}; font-weight: 600;">{performance_level}</span>
                        </p>
                    </div>
                </div>
                ...
                <div style="background-color: #334155; padding: 15px; border-radius: 8px; margin-top: 15px;">
                    ...
                    <ul style="line-height: 1.6; margin-bottom: 10px; padding-left: 20px;">
                        {'<li>🎯 <strong>Ưu tiên:</strong> Giảm số lượng đơn hàng chờ xử lý để cải thiện tỷ lệ hoàn thành</li>' if pending_orders > 10 else ''}
                        {'<li>⚠️ <strong>Cảnh báo:</strong> Tăng cường kiểm soát chất lượng để giảm sự kiện gian lận</li>' if total_fraud > 3 else ''}
                        {'<li>⏱️ <strong>Tối ưu:</strong> Cải thiện hiệu suất thời gian làm việc</li>' if time_efficiency < 70 else ''}
                        {'<li>💰 <strong>Tài chính:</strong> Tập trung vào các đơn hàng có giá trị cao để tăng lợi nhuận</li>' if profit_margin < 15 else ''}
                        {'<li>📊 <strong>Giám sát:</strong> Theo dõi sát sao các cảnh báo nghiêm trọng</li>' if critical_count > 0 else ''}
                        <li>✅ <strong>Duy trì:</strong> Phát huy các điểm mạnh hiện có</li>
                        <li>📈 <strong>Mục tiêu:</strong> Tiếp tục duy trì tỷ lệ hoàn thành trên 95%</li>
                    </ul>

                    <div style="margin-top: 15px; padding: 10px; background-color: #1e293b; border-radius: 5px;">
                        <p style="margin: 0; color: #cbd5e1; font-size: 12px;">
                            <strong>📅 Phạm vi dữ liệu:</strong> Từ {month_names[0]}/{current_year} đến {month_names[current_month - 1]}/{current_year} ({months_passed} tháng)<br>
                            <strong>📊 Nguồn dữ liệu:</strong> SAP Orders & Work Logs<br>
                            <strong>📈 Phương pháp:</strong> Phân tích dữ liệu thực tế & chỉ số KPI<br>
                            <strong>🔄 Cập nhật lần cuối:</strong> {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}
                        </p>
                    </div>
                </div>
            </div>
            ...
            self.analysis_text.setHtml(f"""
                <div style='color: #ef4444; padding: 20px; background-color: #1e293b; border-radius: 8px;'>
                    <h3 style='color: #ef4444;'>⚠️ LỖI KHI TẠO BÁO CÁO</h3>
                    <p>Không thể tạo báo cáo phân tích. Lỗi chi tiết:</p>
                    <pre style='background-color: #0f172a; padding: 10px; border-radius: 5px; color: #cbd5e1;'>{str(e)}</pre>
                    <p style='color: #94a3b8; font-size: 12px; margin-top: 10px;'>Vui lòng kiểm tra dữ liệu đầu vào và thử lại.</p>
                </div>
            """)
def main():
